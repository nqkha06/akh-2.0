  ...
  return (
    <div className="border-b border-border px-4 py-3 last:border-b-0">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-foreground">{item.file.name}</p><p className="mt-0.5 text-xs text-muted-foreground">{formatBytes(item.file.size)} · {item.status === "pending" ? "Đang chờ" : item.status === "uploading" ? `Đang tải lên · ${item.progress}%` : item.status === "success" ? "Hoàn tất" : item.status === "cancelled" ? "Đã hủy" : item.error || "Upload thất bại"}</p></div>
      ...
      {isUploading ? <div className="mt-2.5 h-1.5 overflow-hidden rounded-full bg-muted" role="progressbar" aria-label={`Đang tải ${item.file.name}`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={item.progress}><div className="h-full rounded-full bg-primary transition-[width] duration-200 motion-reduce:transition-none" style={{ width: `${item.progress}%` }} /></div> : null}
    </div>
  );
}
