...
export function LevelsView({
  ...
  async function selectLevel(level: MemberMonetizationLevel) {
    ...
    try {
      ...
      toast.success(
        t("selectionSuccess", {
          level: localizedLevel(level, locale, defaultLocale).name,
        }),
      );
      router.refresh();
    } catch (error) {
    ...
    <div className="mx-auto w-full max-w-[1440px] space-y-6">
      ...
      >
        ...
        <SummaryCard
          ...
          value={
            currentLevel
              ? localizedLevel(currentLevel, locale, defaultLocale).name
              : t("notConfigured")
          }
          description={
...
function LevelCard({
  ...
  const t = useTranslations("SimplePages.levels");
  const content = localizedLevel(level, locale, defaultLocale);
...
function localizedLevel(
  ...
  const translation =
    ...
    level.translations.find((item) => item.locale === defaultLocale) ??
    level.translations.find(
      (item) => item.locale === defaultLocale.split("-")[0],
    ) ??
