...
export function useWithdrawalController() {
  ...
  const validationError = useMemo(() => {
    ...
    if (amount < data.limits.minimum) return `Số tiền tối thiểu là ${formatCurrency(data.limits.minimum)}.`;
  ...
  const formEligible = Boolean(data?.eligibility.eligible && selectedMethod?.verified);
