      ...
      <SheetContent className="w-full gap-0 bg-card p-0 sm:max-w-[460px]">
        ...
        <ScrollArea className="min-h-0 flex-1">
          <div className="p-5">
            ...
            <div className="mt-5 flex items-center justify-between gap-3 rounded-lg border border-border bg-card p-3"><FileStatusBadge status={file.status} /><span className="flex items-center gap-1 text-xs text-muted-foreground">{file.isPublic ? <Globe2 className="size-3.5" /> : <LockKeyhole className="size-3.5" />}{file.isPublic ? "Công khai" : "Riêng tư"}</span></div>
