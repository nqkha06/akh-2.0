...
type LinkStatusFilter = "all" | "active" | "inactive" | "paused";
type LinkTypeFilter = "all" | "url" | "file" | "snippet";
type LinkSort = "newest" | "oldest" | "clicks-desc" | "title-asc" | "actions-desc";
...
function isWithinDateRange(link: LinkDto, createdFrom: string, createdTo: string) {
  ...
  if (createdFrom) {
    const fromTime = new Date(`${createdFrom}T00:00:00`).getTime();
  ...
  if (createdTo) {
    const toTime = new Date(`${createdTo}T23:59:59`).getTime();
...
function filterLinks(links: LinkDto[], filters: LinkFilters) {
  const minClicks = Number(filters.minClicks);

  return links.filter((link) => {
    if (!linkMatchesQuery(link, filters.query)) {
      return false;
    }
    ...
    if (filters.inputType !== "all" && link.inputType !== filters.inputType) {
      return false;
    }

    if (
      filters.platform !== "all" &&
      !link.actions.some((action) => action.platform === filters.platform)
    ) {
      return false;
    }

    if (!isWithinDateRange(link, filters.createdFrom, filters.createdTo)) {
      return false;
    }

    if (filters.minClicks && !Number.isNaN(minClicks) && link.clicks < minClicks) {
      return false;
    }

    if (filters.highPerformance && link.clicks < 500) {
      return false;
    }
...
function sortLinks(links: LinkDto[], sort: LinkSort) {
  return [...links].sort((first, second) => {
    ...
    if (sort === "actions-desc") {
      return second.actions.length - first.actions.length;
    }
