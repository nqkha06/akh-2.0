...
export function MonetizationLevelEditor({
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      if (level) {
        await updateMonetizationLevel(level.id, values);
        toast.success("Đã cập nhật cấp độ kiếm tiền.");
      } else {
        await createMonetizationLevel(values);
          ...
          <div className="overflow-x-auto border-b px-4 sm:px-6">
            ...
            >
              ...
              <TabsTrigger value="vi">
                ...
                <CompletionDot complete={hasTranslation(values, "vi")} />
              </TabsTrigger>
              <TabsTrigger value="en">
                <Languages /> English
                <CompletionDot complete={hasTranslation(values, "en")} />
              </TabsTrigger>
...
function TranslationFields({
  ...
  values,
  setValues,
}: EditorSectionProps & { locale: "vi" | "en"; languageName: string }) {
    ...
    <div className="space-y-5">
      ...
      <FormField label="Tên hiển thị" htmlFor={`level-name-${locale}`}>
        <Input
          ...
          placeholder={locale === "vi" ? "Cân bằng" : "Balanced"}
      ...
      <FormField label="Mô tả" htmlFor={`level-description-${locale}`}>
        <Textarea
          ...
          placeholder={
            locale === "vi"
              ? "Mô tả trải nghiệm và mục tiêu của cấp độ..."
              : "Describe the experience and purpose of this level..."
          }
...
function validate(values: AdminMonetizationLevelPayload) {
  ...
  for (const locale of ["vi", "en"] as const) {
    if (
      ...
      return {
        tab: locale,
        message: `Tên hiển thị ${locale === "vi" ? "Tiếng Việt" : "English"} là bắt buộc.`,
      };
    }
  }
